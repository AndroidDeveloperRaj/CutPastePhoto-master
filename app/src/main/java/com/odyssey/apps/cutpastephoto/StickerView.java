...
public class StickerView extends FrameLayout {
    ...
    if (handlingSticker != null && !locked && (showBorder || showIcons)) {
      ...
      if (showIcons) {
        ...
        for (int i = 0; i < icons.size(); i++) {
          if(i==3 && handlingSticker instanceof TextSticker)continue;
  ...
  protected void onTouchUp(@NonNull MotionEvent event) {
    ...
    if (currentMode == ActionMode.ICON && currentIcon != null && handlingSticker != null) {
      if(currentIcon == icons.get(3)){

        if (onStickerOperationListener != null) {
          onStickerOperationListener.onStickerEdit(handlingSticker);
        }
  ...
  public Drawable currentDrawable(){
    return handlingSticker.getDrawable();
  }
  @NonNull
  ...
  public interface OnStickerOperationListener {
    ...
    void onStickerEdit(@NonNull Sticker sticker);
